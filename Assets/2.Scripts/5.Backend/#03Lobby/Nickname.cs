...
public class Nickname : LoginBase
{
	...
	[SerializeField] Image imageNickname;        // 닉네임 필드 색상 변경 
	[SerializeField] TMP_InputField inputFieldNickname;  // 닉네임 필드 텍스트 정보 추출 

	[SerializeField]  Button btnUpdateNickname;  // "닉네임 설정" 버튼 (상호작용 가능/불가능)
	...
	public void OnClickUpdateNickname()
	{
		...
		ResetUI(imageNickname);
		if (inputFieldNickname.text.Length > 8 && inputFieldNickname.text.Length < 2) return;
	...
	private void UpdateNickname()
	{
		...
		Backend.BMember.UpdateNickname(inputFieldNickname.text, callback =>
		{
			...
			if (callback.IsSuccess())
			{
				...
				// 닉네임 변경에 성공했을 때 onNicknameEvenet에 등록되어 있는 이벤트 메소드 호출 
			...
			else
			{
				...
				{
					case 400:   // 빈 닉네임 혹은 string.Empty, 20자 이상의 닉네, 닉네임 앞/뒤에 공백이 있는 경우 
						message = "닉네임이 비어있거나 | 20자 이상이거나 | 앞/뒤에 공백이 있습니다.";
						break;
					case 409:   // 이미 존재하는 닉네임이 있는 경우 
