...
{
	[SerializeField]
	private Image imageID;              //아이디 필드 색상 변경 
	[SerializeField]
	private TMP_InputField inputFieldID;            // 아이디 필드 텍스트 정보 추출 
	[SerializeField]
	private Image imagePW;              // 비밀번호 필드 색상 변경 
	[SerializeField]
	private TMP_InputField inputFieldPW;            // 비밀번호 필드 텍스트 정보 추출 
	[SerializeField]
			...
			if (callback.IsSuccess())
			{
				...
				Backend.BMember.UpdateCustomEmail(inputFieldEmail.text, callback =>
				{
					if (callback.IsSuccess())
					{
						...
						BackendStoreData.Instance.StoreDataInsert();
