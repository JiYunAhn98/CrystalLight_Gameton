...
public class FindID : LoginBase
{
	...
	{
		// 매개변수로 입력한 InputField UI의 색상과 Message 내용 초기
		ResetUI(imageEmail);

		// 필드 값이 비어있는지 체크 
		if (IsFieldDataEmpty(imageEmail, inputFieldEmail.text, "메일 주")) return;
			...
			else
			{
				...
				switch (int.Parse(callback.GetStatusCode()))
				{
					case 404:   // 해당 이메일의 게이머가 없는 경우 
						message = "해당 이메일을 사용하는 사용자가 없습니.";
						break;
					case 429:   // 24시간 이내에 5회 이상 같은 이메일 정보로 아이디/비번 찾기를 시도한 경우 
